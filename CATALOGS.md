...
| Videos | NeetCode creator-maintained data | Exact video IDs and provenance; no copied transcripts or video |
...
The community index may lag the live platform. Premium markers do not grant access. SQL/JavaScript/shell tasks are indexed and included in the default full-set goal, but need their own execution environments. Some solutions have code only. Empty headings/code fences are excluded. No universally best-video claim is made. Missing videos have an exact-task YouTube search and a place to save your personal choice.

## Refresh

Choose **GitHub → Actions → Refresh problem catalogs → Run workflow**. It also runs when its script/configuration changes. This is a manual/source-change refresh. **Reload catalog snapshots** in Forge reloads published files; it does not refresh upstream sources.
