...
const learning=`<style>${read('frontend/learning.css')}</style>${['four-month-curriculum','solution-demos','selected-walkthroughs'].map(id=>`<script id="${id}" type="application/json">${extraJSON(id)}</script>`).join('')}<script>${read('frontend/learning.js')}</script><script>${read('frontend/compiler-embed.js')}</script>`;
