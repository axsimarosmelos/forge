...
**An optional hosted OneCompiler editor runs its supported languages inside each problem workspace without a personal compiler deployment.** Open it explicitly, load your code, provide input and run in the frame; use “Save embedded draft” to retain received code edits in Forge backups. Frame results are not connected to Forge’s automatic test comparisons. See the [embedding contract](https://onecompiler.com/apis/embed-editor).

**Direct execution through Forge’s own Run/test-suite buttons for C/C++ and additional compiler runtimes needs the separately hosted compiler API and Judge0 service.** Set `JUDGE0_EXTRA_LANGUAGE_IDS=all` to expose installed single-source runtimes, or choose an explicit ID list. The problem editor discovers their names and versions after connection. The service retains fixed limits, so verify runtime compatibility on the deployed provider. There is no claim of literally every programming language, platform hidden-test acceptance, or a solution/video for every indexed task. GitHub Pages hosts the frontend and cannot run this backend. Until connected, study, edit, visualize and download code. Tokens remain in tab memory. See [execution architecture and deployment](ARCHITECTURE.md).
