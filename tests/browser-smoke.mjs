...
}catch(error){let page='';try{await screenshot('failure');page=await evaluate('[document.querySelector(".modal")?.innerText,document.querySelector("#app")?.innerText].filter(Boolean).join("\\n").slice(0,9000)');console.error('PAGE:',page);}catch{}await mkdir(resolve(root,'test-results'),{recursive:true});await writeFile(resolve(root,'test-results/browser-report.json'),JSON.stringify({result:'FAIL',error:error.message,stack:error.stack,page,runtimeErrors},null,2));throw error;}
